...
{
    public class DatabaseHelperContanti
    {
        static object locker = new object(); //Da sostituire eventualmente con costrutto using, serve per gestire gli accessi al db
        SQLiteConnection db; //Variabile per la connessione al db

        public DatabaseHelperContanti()
        {
            //Prendo il percorso del db e, se non esiste, creo la tabella
            db = DependencyService.Get<ISQLite>().GetConnectionContanti();
        ...
        //Funzione per cancellare interamente la tabella contanti
        public int DropContanti()
        {
            lock (locker)
            {
